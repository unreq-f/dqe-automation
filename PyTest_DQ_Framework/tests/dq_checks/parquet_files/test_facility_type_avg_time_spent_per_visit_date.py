"""
Description: Data Quality checks facility_type_avg_time_spent_per_visit_date
...
def source_data(db_connection):
    source_query = """
    select  f.facility_type, date(v.visit_timestamp) as visit_date, ROUND(AVG(duration_minutes), 2) AS avg_time_spent
...
@pytest.mark.parquet_data
@pytest.mark.facility_type_avg_time_spent_per_visit_date
def test_check_data_completeness(source_data, target_data):
    source_data['visit_date'] = pd.to_datetime(source_data['visit_date']) #hardcode for made merge possible
    DataQualityLibrary.check_missing_rows(source_data, target_data,  key_columns=['facility_type', 'visit_date','avg_time_spent'])